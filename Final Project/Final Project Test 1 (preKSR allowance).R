...
# for the first test, we will be looking at allowance factors for applications filed prior to KSR, therefore, as we did with the transaction history, we will narrow our data to applications for which postKSR=0
patents <- patents[patents$postKSR ==0,]
...
patents.allowed.train.rf = randomForest(as.factor(allowed) ~ RCE + rejections + examinerInterviews + continuity + foreign + as.Date(filingDate), data=patents,ntree=5000, importance=TRUE)
...
# we want to know what that significat date is so we use:
as.Date(11530)
# significant date is 2001-07-27
